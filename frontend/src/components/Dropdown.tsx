...
const Dropdown = ({
  ...
  return (
    ...
    >
      <button className="relative text-gray-700 dark:text-gray-300 hover:text-sky-600 dark:hover:text-sky-400 px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-300 hover:bg-sky-50 dark:hover:bg-sky-900/20 flex items-center gap-1">
        {title}
        <HiChevronDown
          ...
          <div className="px-2">
            {items.map(({ label, icon, to }, index) => (
              <Link
                key={label}
                to={to}
                className="group/item flex items-center gap-4 px-4 py-3 mx-1 text-sm text-gray-700 dark:text-gray-300 hover:text-sky-600 dark:hover:text-sky-400 hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 dark:hover:from-blue-900/20 dark:hover:to-indigo-900/20 rounded-xl transition-all duration-300 transform hover:scale-[1.02] hover:shadow-md"
              >
                ...
                <div className="flex-1">
                  ...
                  {descriptions[index] && (
                    <div className="text-xs text-gray-500 dark:text-gray-400 group-hover/item:text-sky-400 transition-colors">
                      {descriptions[index]}
                    </div>
                  )}
                </div>
